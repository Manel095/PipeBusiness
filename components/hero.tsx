...
export function Hero() {
  ...
  // Phase 0: Node 1 fades in (0 to 0.1)
  const node1Opacity = useTransform(scrollYProgress, [0, 0.1], [0, 1])
  const node1Y = useTransform(scrollYProgress, [0, 0.1], [50, 0])

  // Phase 0.5: Webhook modal pops up briefly (0.1 to 0.15, disappears at 0.3)
  const webhookOpacity = useTransform(scrollYProgress, [0.1, 0.15, 0.25, 0.3], [0, 1, 1, 0])
  const webhookScale = useTransform(scrollYProgress, [0.1, 0.15, 0.25, 0.3], [0.8, 1, 1, 0.8])

  // Phase 1: Node 2 fades in (0.3 to 0.4)
  const node2Opacity = useTransform(scrollYProgress, [0.3, 0.4], [0, 1])
  const node2X = useTransform(scrollYProgress, [0.3, 0.4], [50, 0])

  // Phase 1.5: Connection line draws and Data mapping modal appears
  const lineWidth = useTransform(scrollYProgress, [0.4, 0.45], ["0%", "100%"])
  const mappingOpacity = useTransform(scrollYProgress, [0.45, 0.5, 0.55, 0.6], [0, 1, 1, 0])
  const mappingScale = useTransform(scrollYProgress, [0.45, 0.5, 0.55, 0.6], [0.8, 1, 1, 0.8])

  // Phase 2: Data starts flowing
  const flowOpacity = useTransform(scrollYProgress, [0.6, 0.65], [0, 1])

  // Phase 3: BI Dashboard drops in
  const biOpacity = useTransform(scrollYProgress, [0.7, 0.8], [0, 1])
  const biY = useTransform(scrollYProgress, [0.7, 0.8], [50, 0])

  return (
    <section ref={containerRef} className="relative h-[400vh] w-full" id="top">
      <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-start overflow-hidden bg-background">
        ...
        <div className="relative w-full max-w-6xl px-5 flex flex-col items-center justify-start h-full pt-20 md:pt-32">
          
          {/* Main Title that stays during the scroll but fades out as the animation progresses */}
          <motion.div 
            style={{ opacity: useTransform(scrollYProgress, [0, 0.3], [1, 0]), y: useTransform(scrollYProgress, [0, 0.3], [0, -50]) }}
            className="w-full text-center z-20 pointer-events-auto"
          >
          ...
          {/* The Canvas Area (Animates in as user scrolls) */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none mt-20">
            <div className="relative w-full max-w-5xl h-[500px] flex items-center justify-center">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between w-full max-w-3xl gap-20 md:gap-0 px-4">
                ...
                >
                  ...
                  <motion.div 
                    ...
                    className="absolute -top-16 -right-5 md:-right-16 bg-surface border border-border rounded-xl p-3 shadow-xl z-20 w-48"
                  >
                ...
                <motion.div
                  ...
                  className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-[120px] bg-surface border border-border rounded-xl p-4 shadow-xl z-30 w-56 flex flex-col items-center"
                >
              ...
              <motion.div 
                ...
                className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-3xl bg-background border border-border rounded-2xl p-6 shadow-2xl z-40"
              >
