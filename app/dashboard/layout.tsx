...
import { DEMO_PROCESSES, DEMO_CONNECTORS } from "@/lib/demo-data"
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!initialized) {
      actions.init({ processes: DEMO_PROCESSES, connectors: DEMO_CONNECTORS })
      setInitialized(true)
    }
  }, [initialized])
      ...
      <aside className="dashboard-sidebar">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${
                  isActive ? "bg-brand/10 text-brand" : "text-muted-foreground hover:bg-surface hover:text-foreground"
                }`}
              >
        ...
        <div className="mt-6 px-3">
          <div className="flex items-center justify-between px-3 mb-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                actions.addProcess({
                  id, name: "New Engine", description: "",
                  position: { x: 300 + Math.random() * 200, y: 200 + Math.random() * 200 },
                  color: "#FF0083", dataSources: [], data: [], incomingData: [],
          ...
          <div className="flex flex-col gap-0.5">
            {workspace.processes.map((proc) => (
              ...
              >
                <span
                  className="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold uppercase flex-shrink-0"
        ...
        <div className="mt-auto border-t border-border px-3 py-3">
          <div className="flex items-center gap-3 rounded-xl px-3 py-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand/10 text-sm font-bold text-brand">M</div>
      ...
      <div className="dashboard-main">
        <div className="dashboard-topbar">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand text-sm font-bold text-brand-foreground">M</div>
          </div>
        </div>
